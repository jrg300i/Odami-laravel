...
@section('title', 'Editar Factura ' . $factura->numero_completo . ' - Tapicería Odami')
...
<div class="max-w-5xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-gray-600">Factura {{ $factura->numero_completo }}</p>
        </div>
        <a href="{{ route('facturas.show', $factura) }}" class="text-gray-600 hover:text-gray-900">
    ...
    <form action="{{ route('facturas.update', $factura) }}" method="POST" id="facturaForm">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            <div class="lg:col-span-2 bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div>
                        ...
                        <select name="serie" id="serie" class="w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500" required>
                            @foreach($series as $s)
                                <option value="{{ $s->serie }}" {{ $factura->serie == $s->serie ? 'selected' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="concepto" id="concepto" value="{{ old('concepto', $factura->concepto) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div>
                        ...
                        <select name="forma_pago" id="forma_pago" class="w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                            ...
                            <option value="transferencia" {{ $factura->forma_pago == 'transferencia' ? 'selected' : '' }}>Transferencia bancaria</option>
                            <option value="efectivo" {{ $factura->forma_pago == 'efectivo' ? 'selected' : '' }}>Efectivo</option>
                            <option value="tarjeta" {{ $factura->forma_pago == 'tarjeta' ? 'selected' : '' }}>Tarjeta</option>
                            <option value="bizum" {{ $factura->forma_pago == 'bizum' ? 'selected' : '' }}>Bizum</option>
                            <option value="cheque" {{ $factura->forma_pago == 'cheque' ? 'selected' : '' }}>Cheque</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="iva" id="iva" class="w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500" required>
                            <option value="21" {{ $factura->iva == 21 ? 'selected' : '' }}>21% (General)</option>
                            <option value="10" {{ $factura->iva == 10 ? 'selected' : '' }}>10% (Reducido)</option>
                            <option value="4" {{ $factura->iva == 4 ? 'selected' : '' }}>4% (Superreducido)</option>
                            <option value="0" {{ $factura->iva == 0 ? 'selected' : '' }}>0% (Exento)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    <div class="flex items-center">
                        <input type="checkbox" name="incluir_clausulas" id="incluir_clausulas" value="1"
                               {{ $factura->incluir_clausulas ? 'checked' : '' }}
                ...
                <div class="mb-4">
                    ...
                    <textarea name="observaciones" id="observaciones" rows="2"
                              class="w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">{{ old('observaciones', $factura->observaciones) }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-medium" id="subtotalDisplay">{{ number_format($factura->subtotal, 2) }} €</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600">IVA (<span id="ivaDisplay">{{ $factura->iva }}</span>%):</span>
                        <span class="font-medium" id="ivaAmountDisplay">{{ number_format($factura->total - $factura->subtotal, 2) }} €</span>
                    </div>
                    <div class="border-t pt-3 flex justify-between">
                        <span class="text-lg font-semibold text-gray-900">Total:</span>
                        <span class="text-lg font-bold text-amber-600" id="totalDisplay">{{ number_format($factura->total, 2) }} €</span>
                    </div>
                </div>

                <div class="mt-6 p-4 bg-amber-50 rounded-lg">
                    <p class="text-sm text-amber-800">
                        <strong>Nota:</strong> Los totales se recalcularán automáticamente al guardar según las líneas de factura.
                    </p>
                </div>
            </div>
        </div>
                    ...
                    <tbody id="lineasBody">
                        @if($factura->lineas && count($factura->lineas) > 0)
                            ...
                            <tr class="linea-row border-b border-gray-100">
                                <td class="py-2 px-2">
                                    <input type="text" name="lineas[{{ $index }}][descripcion]"
                                ...
                                <td class="py-2 px-2">
                                    <input type="number" name="lineas[{{ $index }}][cantidad]"
                                           value="{{ $linea['cantidad'] }}" min="0.01" step="0.01"
                                ...
                                <td class="py-2 px-2">
                                    <input type="number" name="lineas[{{ $index }}][precio]"
                                           value="{{ $linea['precio'] }}" min="0" step="0.01"
                                ...
                                <td class="py-2 px-2 text-right font-medium total-linea">
        ...
        <div class="flex justify-end gap-3">
            <a href="{{ route('facturas.show', $factura) }}" class="px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">
                Cancelar
            </a>
...
document.addEventListener('DOMContentLoaded', function() {
    let lineaIndex = {{ isset($index) ? $index + 1 : 1 }};
